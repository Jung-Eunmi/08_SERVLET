...
public class RedirectServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        *   따라서 Redirect 되는 서블릿은 doGet 메소드를 사용해서 처리해야한다.
        * */
        ...
        /* comment.
        *   출력을 했을 때 firstName = null, lastName = null 라고 값이 확인되었다.
        *   따라서 redirect 시에는 request 가 2번 이루어지기 때문에
        *   1번째 request 와 2번째 request 는 전형 다른 객체가 된다. => 값 공유 불가능
        * */
        ...
        *   그렇기 때문에 개인 PC 는 상관이 없지만 공용 PC 의 경우 탈취 위험에 노출된다.
        *   따라서 민감한 정보는 session 을 이용하게 되며 session 은 클라이언트 컴퓨터가 아닌
        *   서버 컴퓨터 (톰켓서버)에 관리 되기 때문에 보안에 더 우수하다.
        * */
